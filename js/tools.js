    ...
    renderDetailMonth: function (dayWrapper, recivedYear, recivedMonth) {
        ...
        if (month !== recivedMonth) {
          if (date.getDay() === 0 || date.getDay() === 6) {
            array.push({
              ...
              state: 'weekend weekend-before-after',
      ...
      for (var j = 0; j < array.length; j++) {
       ...
       /* _html += `<li data-time="${array[j].dateStr}" class="${array[j].state}">
                  ...
                  var len = array[j].dateStr.length;
                  var getMonth =(Number(array[j].dateStr.substring(len-4,len-2))+1).toString();
                  var getDate = (Number(array[j].dateStr.substring(len-2,len))).toString();
                  var getYear = (Number(array[j].dateStr.substring(0,len-4)))
                  ...
                  var realDateTime = getYear+getMonth+getDate;
                  //console.log(getYear,getMonth,getDate);
                 _html +='<li  data-time='+array[j].dateStr+' class="'+array[j].state+'" real-time="'+realDateTime+'" year="'+getYear+'" month="'+getMonth+'" getDate="'+getDate+'">';
                 _html+= '<p class="info"><span class="date">';
                 _html += '<em>'+array[j].day+'</em>日';
                 _html+='</span></p>';
                 //_html += '<p class="'+festival_state+'">'+array[j].festival+'</p>';
               // console.log('<li  data-time='+array[j].dateStr+' class="'+array[j].state+'"></li>');
                // _html += '<p  class="'+term_state+'">'+array[j].term+'</p>';
                 _html+='</li>'; 
      }
